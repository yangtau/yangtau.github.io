...
Interpreter 中`compile`实现分析树的构建, `interpret`实现语句的执行. 因为全局只需要一个 interpreter, 所以别的地方可以通过`getCurrentInterpreter`获取当前interpreter.
...
<!--当我第一次看到屏幕上输出`Hello World`的时候, 内心有点激动, 有点像看到自己刚出生的孩子的感觉(虽然女朋友都没有).-->
