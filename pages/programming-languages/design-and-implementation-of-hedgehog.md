...
还有其他一些小细节应该注意，例如多个连在一起的换行符。显然不是每一个换行都对应着一条语句。可能是多对一的情况。还有一种可能在文件结尾根本就没有换行符，但是最后一条语句还是应当被解析。
...
```
第一段代码输出一行后退出，而第二段代码输出会三行。
所以一般 JS 的代码规范中都会要求语句结束写 `;`。

想要完美地去掉 `;` 真不是一件容易的事啊。我目前的想法是 `+` 这种二元操作符显然不能作为语句的结束，就忽略其后的换行符。
而对于 `+` 在语句开头这种情况就输出警告信息提醒程序员(因为这种写法虽然没有意义，但是在语法层面上是没有问题的)。
